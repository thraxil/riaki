...
class Page:
    ...
    def versions(self):
        return [v.get() for v
                in self._page.get_links()
                if v.get_bucket() == "riakiversion"
                and v.get().exists()]

    def version_data(self, v):
        try:
            d = loads(v.get_data())
            d['version_created'] = datetime.strptime(d['version_created'],
                                                     DTFORMAT)
    ...
    def link_text(self, text=""):
        ...
        for part in pattern.split(text):
            ...
            if m:
                ...
                if cnt:
                    part = ("""<a href="/page/%s/" """
                            """class="existing">%s</a>""") % (slug, link_text)
                else:
                    part = ("""<a href="/page/%s/" """
                            """class="notfound">%s</a>""") % (make_slug(title),
                                                              link_text)
            results.append(part)
        return ''.join(results)
...
def get_tag_pages(tag):
    ...
    if not t.exists():
        return []
    return [Page(p.get()) for p in t.get_links()
            if p.get_bucket() == "riakipage"]


def get_all_tags():
    tagindex = index_bucket.get_binary('tag-index')
    tags = [str(t.get().get_data()) for t in tagindex.get_links()
            if t.get().exists()]
...
def get_all_pages():
    pageindex = index_bucket.get_binary('page-index')
    pages = [Page(p.get_binary()) for p in pageindex.get_links()
             if p.get().exists()]
